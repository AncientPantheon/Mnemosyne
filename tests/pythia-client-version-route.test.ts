...
import { GET } from "../app/api/admin/pythia-client-version/route";
import { signSession } from "../lib/auth/session";

const SECRET = "pythia-client-version-route-test-session-secret!!";

function req(cookie?: string): Request {
  return new Request("http://localhost:3005/api/admin/pythia-client-version", {
    method: "GET",
...
describe("/api/admin/pythia-client-version — ancient-gated scaffold", () => {
  ...
  it("200s for an ancient with { installed:<semver>, wired:true, available } + no-store", async () => {
    ...
    // Pythia-client is a real dependency: installed is read from node_modules, wired:true.
  ...
  it("flags updateAvailable when npm publishes a newer Pythia-client", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
