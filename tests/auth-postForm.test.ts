...
describe("postForm — token-exchange redirect handling", () => {
  it("follows a 308 trailing-slash redirect, preserving method, body, and Authorization", async () => {
    const calls: Array<{ url: string; init: RequestInit }> = [];
    const fetchMock = vi.fn(async (url: string, init: RequestInit) => {
      calls.push({ url, init });
      if (calls.length === 1) {
        // The hub's Next.js trailingSlash 308 — Node's auto-follow would drop
        // the POST body + auth here, which the IdP then rejects.
    ...
    // ...and carries the method + body + Authorization the auto-follow would drop.
  ...
  it("returns the response directly (single call) when there is no redirect", async () => {
