...
/**
 * The subset of the hub's OIDC discovery document Mnemosyne consumes. Endpoints +
 * capabilities are FEATURE-DETECTED from `/.well-known/openid-configuration`
 * rather than hardcoded, so an additive hub change (e.g. refresh tokens) never
 * breaks us silently.
 */
...
/**
 * Resolve (and memoise) the hub's discovery document + a JWKS key resolver for an
 * issuer. The `clock` is injectable so tests can exercise TTL expiry without real
 * time.
 *
